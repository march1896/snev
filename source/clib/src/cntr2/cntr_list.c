...
static unknown cntr_list_cast(unknown x, unique_id intf_id) {
	...
	switch (intf_id) {
	case CNTR_BASE_INTERFACE_ID:
		return (unknown)&c->__iftable[CNTR_BASE_INTERFACE_OFFSET];
	case CNTR_LINEAR_INTERFACE_ID:
		return (unknown)&c->__iftable[CNTR_LINEAR_INTERFACE_OFFSET];
	default:
		return NULL;
	}
