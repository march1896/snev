...
struct embeded_link_t {
	struct embeded_link_t* prev;
	struct embeded_link_t* next;
};

typedef embeded_link_t emlink;

struct embeded_link_list_t {
	unsigned int size;
	unsigned int flags;

	embeded_link_t ssent;  
	embeded_link_t esent;

	bool (*compare_object)(void* lhs, void* rhs);
	void *compare_param;
	bool (*find_object)(void* obj);
	void *find_param;
};
