...
static void __as_set_basic_test() {
	iset set = as_set(cntr_create_ollrb_v(int_compare, __global_default_allocator, NULL));
	set_test_basic(set);
	iset_destroy(set);
}

static void __as_mset_basic_test() {
	imset mset = as_mset(cntr_create_ollrb_v(int_compare, __global_default_allocator, NULL));
