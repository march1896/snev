...
typedef struct _Block {
	struct _Block* pPrevMem;
	struct _Block* pNextMem;
	struct _Block* pPrevFree;
	struct _Block* pNextFree;
...
static inline int CheckPointer( Block* b, int ForFree );
static inline int CheckBlock( Block* b );
static inline int IsBlockFree( Block* b );
...
void InitHeap( void *Buffer, size_t Size, uint Alignment ) {
	...
	pHeapStart = Buffer;
	AlignMent = Alignment;
	for ( i = 0; i < 32; i ++ ) pFreeList[i] = NULL;
...
int CheckPointer( Block*b, int ForFree ) {
	if ( b == NULL ) return 1;
	if ( !ForFree ) {
...
int CheckBlock( Block* b ) {
	if ( b == NULL ) return 0;
	if ( b <= pStartSentinel || b >= pEndSentinel ) return 0;

	int ret = 1;
	ret &= CheckPointer( b->pPrevMem, 0 );
	ret &= CheckPointer( b->pNextMem, 0 );
	ret &= CheckPointer( b->pPrevFree, 1 );
	ret &= CheckPointer( b->pNextFree, 1 );

	return ret;
}

int IsBlockFree( Block* b ) {
	if ( !CheckBlock( b ) ) return 0;

	if ( b->pPrevFree || b->pNextFree ) return 1;
	return 0;
}
